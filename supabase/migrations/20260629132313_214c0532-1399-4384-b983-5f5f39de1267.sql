...
AS $function$
  SELECT public.is_active_user(_user_id)
    AND (
      ...
      -- Legacy/orphan portfolio: no areas linked → visible to anyone with portfolio:view
      OR (
        public.has_module_perm(_user_id, 'portfolio', ARRAY['view','edit','admin'])
        AND NOT EXISTS (SELECT 1 FROM public.portfolio_area pa WHERE pa.portfolio_id = _portfolio_id)
      )
      OR (
