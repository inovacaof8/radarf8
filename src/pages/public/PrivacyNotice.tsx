...
export default function PrivacyNotice() {
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const { data: doc } = await supabase.from("legal_documents").select("id").eq("type", "privacy").single();
      if (!doc) return null;
  ...
  return (
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-3xl mx-auto">
        ...
        <h1 className="text-2xl font-bold text-foreground mb-2">Aviso de Privacidade</h1>
